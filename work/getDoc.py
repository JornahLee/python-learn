...
def generate_dto(field_list: list):
    is_response = False
...
def generate_params(field_list: list):
    ...
    * {}
    {}
    */
    public String xxxx(String advertiserId,{}){{
    String url = "{}";
    URI uri = getUri(url, advertiserId);
...
if __name__ == '__main__':
    page_url = 'https://open-ad.vivo.com.cn/doc/index?id=186'
    accessor = 'private'
    global api_url, api_title
    with open('xxx.html', encoding='utf-8') as f:
        extract_res = extract_field()
        generate_dto(extract_res)
        print('----------------------------------------------------')
        print('----------------------------------------------------')
