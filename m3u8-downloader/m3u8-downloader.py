...
######################配置信息##########################
# m3u8链接批量输入文件
m3u8InputFilePath = "./input/m3u8s_input.txt"
# 视频保存路径
...
def getM3u8Info():
    ...
    while True:
        ...
        try:
            ...
            expected_length = int('0' if response.headers.get('Content-Length') is None else response.headers.get('Content-Length'))
            actual_length = len(response.content)
            if expected_length > actual_length:
                # raise Exception("m3u8下载不完整")
                print('warning!!!  m3nu 可能下载不完成.....')
